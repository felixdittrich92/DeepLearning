...
ReduceLearningRateOnPlateau: verändert die Learning Rate nur, wenn die Entwicklung nicht besser wird

-> besser als LRScheduler
'''
...
# Log Dir
log_dir = os.path.abspath("../DeepLearning/logs/ReduceLROnPlateau")
...
plateau_callback = ReduceLROnPlateau(
    monitor='val_loss',  # welcher loss soll überprüft werden
    factor=0.95, # um wieviel learning rate reduzieren -> new lr = lr * factor
    patience=3, # wenn 3 mal nicht verbessert dann anpassen
    verbose=1,
    min_lr=1e-5) # minimum

# Tensorboardobjekt wird überschrieben und die Lernrate wird mit abgespeichert
...
model_log_dir = os.path.join(log_dir, "modelPlateau")
...
rand_model.fit(
    ...
    epochs=epochs, 
    callbacks=[tb_callback, plateau_callback], # an Tensorboard übergeben
